...
class TestImageShare(TestCase):
    ...
    def test_image_share_creation(self, mock_prune_old_image_share):
        ...
        response = self.client.post(reverse('images_share'),
                                    {'object_type': 'wall', 'status': 'accepted', 'remarks': 'I am requesting images.'})
        ...
        response = self.client.post(reverse('images_share'),
                                    {'id': image_share.id, 'object_type': 'wall', 'status': 'accepted',
                                     'remarks': 'I am requesting images.'})
        self.assertEqual(response.status_code, 302)
        ...
        self.assertIsInstance(response.context['images_share'],
                              Page)  # Page (Paginator specific page) is sent in render
...
class TestCrowdSource(TestCase):
    ...
    def test_crowd_source_crud(self, mock_upload_object_while_loggin_in, mock_delete_object, mock_upload_object,
                               mock_move_object, mock_save_image_file, mock_delete_image_file):
        with open(finders.find('dist/img/avatar.png'), 'rb') as image:
            # Guest User
            response = self.client.post(reverse('crowdsource'),
                                        {'object_type': 'wall', 'image_type': 'raw', 'file': image})
            self.assertEqual(response.status_code, 302)
            ...
            # Login as Admin
            self.client.force_login(self.admin)
            crowdsource = Crowdsource.objects.filter(object_type='wall', image_type='raw').order_by(
                '-created_at').first()
            response = self.client.post(reverse('crowdsource'),
                                        {'id': crowdsource.id, 'object_type': 'facade', 'image_type': 'raw'})
            self.assertEqual(response.status_code, 302)
            ...
            response = self.client.post(reverse('crowdsource'),
                                        {'object_type': 'test', 'image_type': 'raw', 'file': image,
                                         'direct_upload': True})
            self.assertEqual(response.status_code, 302)
        ...
        self.assertIsInstance(response.context['crowdsources'],
                              Page)  # Page (Paginator specific page) is sent in render
        self.assertEqual(response.context['query'], "test")  # Query parameteter value (q) is sent back
